            ...
            <div className="flex flex-col items-start h-full justify-start">
                ...
                {isBaseline ? (
                    ...
                    <div className="space-y-2">
                        ...
                        {/* 1. Clinical Hours Repurposed (Revenue Opportunity) - HIDDEN IF BLUE BOX IS PRESENT */}
                        {!metrics.freedSupervisorHours && (
                            <div className="flex justify-between items-center group relative cursor-help">
                                <div className="flex items-center gap-1">
                                    <span className="text-sm text-slate-600 dark:text-slate-300">+ Clinical Hours Repurposed</span>
                                    <span className="text-slate-400 text-[10px]">(?)</span>
                                </div>
                                <span className="font-mono font-medium text-emerald-600 dark:text-emerald-400">+{formatMoney(metrics.realizedRevenue)}</span>
                                ...
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-slate-800 text-white text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                                    Projected revenue from {globalDefaults.reinvestmentTask || "Outpatient Counseling"}.
                                </div>
                            </div>
                        )}
